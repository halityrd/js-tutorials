var urunAdi = "iphone 13"; // string
let urunFiyat = 15000; // number

console.log(typeof urunAdi);
console.log(typeof urunFiyat);

// let sayi1 = "10";
// let sayi2 = "20";

//console.log(Number(sayi1) + Number(sayi2)); // String değişkenden Number değişkene tür dönüşmü

let sayi1 = 10;
let sayi2 = 20;

console.log(sayi1.toString() + sayi2.toString()); // Number değişkenden String değişkene tür dönüşümü

let isim = "sena";
let soyad = "turan";

console.log(isim + " " + soyad);

let sinavNotu = 40;

let basarilimi = (sinavNotu >= 50);

console.log(basarilimi);
console.log(typeof basarilimi); //boolean

let yas;
yas;

console.log(yas);
console.log(typeof yas); // undefined

