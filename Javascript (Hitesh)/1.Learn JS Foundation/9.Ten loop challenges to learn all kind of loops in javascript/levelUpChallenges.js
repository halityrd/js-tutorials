...
`[1, 2, 3, 4, 5]` and stop when the number `4` is found.
    Store he numbers before `4` in an array named
    `smallNumbers`.
*/
...
`["chai", "grean tea", "herbal tea", "black tea"]` and
skip `"herbal tea"`.
    Store the other teas in an array amed `preferredTeas`.
*/
...
containing city populations.
    ...
    store the res in a new object named `largeCities`.
...
`["early grey", "green tea", "chai", "oolong tea"]`.
...
`[2, 5, 7, 9]`.
  Skip the value `7` and muliply the rest by 2. Store
  the results in a new array named `doubleNumbers`.
*/
...
`["chai", "green tea", "black tea", "jasmine tea", "herbal tea"]`
  ...
  greate than 10.
  Store the teas iterated over in array named
  `shortTeas`.
*/
